...
| `webapp/app.py::_adaptive_json` | REPORTING | `adaptive/engine.py::build` → the same object `_intro_page` renders | **DECLARED — v6.** It serves a business-model classification, a customer job, a ranked decision map and an abstention reason: every one of those is a statement about the COMPANY, so it is not diagnostics and is not exempt beside `/timing` and `/telemetry`. It composes nothing of its own — it is the machine-readable projection of the object the intro page renders, so it cannot disagree with the page, and a qualification matrix reading it is reading what the customer was shown. Owner-gated like every other run surface |
| `adaptive/engine.py::build` | CANONICAL_ADAPTIVE | `executive/company_profile.profile_for` (three ranked classifiers) + this run's evidence | **CANONICAL — v6.** `strategic_read.compose` answers *what is the reading*; this answers *which reading applies to this company and why that one*. It asserts no thesis and never decides whether a reading exists — a company it cannot classify gets a stated refusal and the general reading, labelled |
