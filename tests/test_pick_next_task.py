...
def test_parse_real_roadmap_file_has_the_expected_runnable_tasks():
    ...
    # T010-T020 (Decision Record + wiring; report 2A/2B; Company Event
    # System; CRM; analytics; knowledge promotion; marketing C3-C8;
    # growth platform; research & evidence; product strategy & roadmap)
    # completed 2026-07-20/21. The current queue is T021 (Executive
    # Decision Agent, recommend-only). NEEDS-SPEC items are still never
    # auto-promoted.
    assert runnable_ids == {"T021"}
    # The nightly loop must pick T021 (the only RUNNABLE task), never a
    # NEEDS-SPEC item.
    assert pick_next_runnable(tasks) == "T021"
