...
def test_live_effects_span_one_write_cycle_not_seven_months():
    ...
    # `cycles_with_effects < len(months)` was true when the engine had written
    # effects in one cycle over a corpus whose `created_at` spanned seven
    # months -- the whole point being that the segmenter must NOT read the
    # date field. Production has since run for months (measured 2026-08-22:
    # 28 write cycles over 7 months), so the inequality reverses without
    # anything regressing, exactly as this test's own docstring predicted.
    #
    # The durable claim is the mechanism, and it is asserted below on a
    # constructed ledger where the two disagree by construction. Against the
    # live ledger only the sanity floor is checked.
    assert cycles_with_effects >= 1


def test_effect_cycles_follow_append_order_not_the_created_at_field():
    """The negative control the live assertion used to be, made unfalsifiable
    by history and rebuilt so it can always fail.

    Two effects written in ONE cycle, carrying `created_at` seven months
    apart. A segmenter that keyed on the date would report two cycles.
    """
    ledger = [
        {"record": "cycle_start", "cycle_id": "c1"},
        {"record": "knowledge_effect", "created_at": "2026-01-05"},
        {"record": "knowledge_effect", "created_at": "2026-08-05"},
    ]
    segments = LA._effect_cycles(ledger)
    assert sum(1 for c in segments if c) == 1, (
        "two effects appended in one cycle were split across cycles; the "
        "segmenter is reading a date field instead of append order")
    months = {r["created_at"][:7] for r in ledger
              if r["record"] == "knowledge_effect"}
    assert len(months) == 2, "the fixture must actually span two months"
