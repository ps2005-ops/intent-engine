"""Break proofs for the V3 closure run.
...
DM  = f"{T}/test_market_durable_memory.py"
RPR = f"{T}/test_market_research_priorities.py"
DEP = f"{T}/test_market_dependencies.py"
VRD = f"{T}/test_market_v3_readiness.py"
...
# --- V3 closure ----------------------------------------------------
    ("v3-1. a conflicting adjudication silently overwrites the first",
     S / "learning_store.py",
     "                self._append(kind, {**payload, \"supersedes\": identity,\n"
     "                                    \"conflict_on\": field_name})\n"
     "                return \"conflict\"",
     "                return \"held\"",
     f"{DM}::test_a_conflicting_adjudication_is_appended_never_overwritten"),

    ("v3-2. an unobservable falsifier is accepted",
     S / "falsifiers.py",
     "    if _UNOBSERVABLE.search(text):\n        return REFUSED_UNOBSERVABLE",
     "    if False:\n        return REFUSED_UNOBSERVABLE",
     f"{DM}::test_a_falsifier_naming_no_observation_is_refused"),

    ("v3-3. an expired falsifier window counts as survival",
     S / "falsifiers.py",
     "    return Falsifier(**{**falsifier.__dict__, \"standing\": EXPIRED,",
     "    return Falsifier(**{**falsifier.__dict__, \"standing\": RESOLVED_SURVIVED,",
     f"{DM}::test_an_expired_window_is_not_survival"),

    ("v3-4. a watch is opened with no preregistration behind it",
     S / "response_watch.py",
     "    if not expectation_id.strip():",
     "    if False:",
     f"{DM}::test_a_watch_without_a_preregistration_is_refused"),

    ("v3-5. an observed episode is marked preregistered",
     S / "learning_store.py",
     "        if payload.get(\"preregistered\"):",
     "        if False:",
     f"{DM}::test_an_observed_episode_cannot_claim_it_was_preregistered"),

    ("v3-6. a research query commissions a conclusion",
     S / "research_priorities.py",
     "    if _LEADING.search(question) or _LEADING.search(why_it_matters or \"\"):",
     "    if False:",
     f"{RPR}::test_a_leading_rationale_is_refused"),

    ("v3-7. a buyer question is routed to release notes",
     S / "research_priorities.py",
     "    NEED_BUYER: (\"pricing_page\", \"solution_page\", \"customer_launch_page\",\n"
     "                 \"plan_comparison\"),",
     "    NEED_BUYER: (\"pricing_page\", \"release_notes\"),",
     f"{RPR}::test_a_buyer_question_does_not_go_to_release_notes"),

    ("v3-8. using a platform is recorded as depending on it",
     S / "dependencies.py",
     "    if _USAGE.search(text):\n        return USES, \"\"",
     "    if _USAGE.search(text):\n        return DEPENDS_ON, \"\"",
     f"{DEP}::test_running_on_a_platform_is_use_not_dependency"),

    ("v3-9. a lone sentence-opening capital becomes a supplier",
     S / "dependencies.py",
     "        if hit.start() == 0 and len(hit.group(1).split()) == 1:\n            continue",
     "        if False:\n            continue",
     f"{DEP}::test_a_lone_sentence_opening_capital_is_not_a_supplier"),

    ("v3-10. a stage nobody fed is reported as the bottleneck",
     S / "v3_readiness.py",
     "        return (self.outputs / self.inputs) if self.inputs else None",
     "        return (self.outputs / self.inputs) if self.inputs else 0.0",
     f"{VRD}::test_a_stage_nobody_fed_is_not_the_loudest_bottleneck"),

    ("v3-11. a readiness axis passes with no measured reason",
     S / "v3_readiness.py",
     "    unreasoned = [a.axis for a in axes if not a.reason.strip()]",
     "    unreasoned = []",
     f"{VRD}::test_an_axis_with_no_measured_reason_is_flagged"),
...
def main() -> int:
    return run_all([Proof(*row) for row in PROOFS],
                   title="V3 closure break proofs, hardened harness")
